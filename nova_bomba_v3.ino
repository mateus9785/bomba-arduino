...
void ActivateNextLed() {
    ...
    if (successState || errorCount >= 3) return;
...
void CheckLedStatus() {
    if (ledsActive && millis() >= ledOffTime) {
        ...
        if (errorCount >= 1) digitalWrite(LED_ERROR1_PIN, HIGH);
        if (errorCount >= 2) digitalWrite(LED_ERROR2_PIN, HIGH);
        if (errorCount >= 3) digitalWrite(LED_ERROR3_PIN, HIGH);
        
        // Green LED should stay on if in success state
        if (!successState) {
            for (int i = 0; i < numLeds; i++) {
                if ((ledPins[i] == LED_ERROR1_PIN && errorCount < 1) ||
                    (ledPins[i] == LED_ERROR2_PIN && errorCount < 2) ||
                    (ledPins[i] == LED_ERROR3_PIN && errorCount < 3)) {
                    digitalWrite(ledPins[i], LOW);
                }
            }
        }
...
int getCorrectWireIndex() {
  ...
  switch (uncutWiresCount) {
    ...
    case 1: 
      // When only one wire is left, return that wire
      for (int i = 0; i < 8; i++) {
        if (wireStates[i]) {
          return i; // Return the index of the only remaining wire
        }
      }
      
    default: return -1; // No more wires to cut
  }
}
...
void printWireStates() {
  ...
  if (correctWireIndex >= 0) {
    ...
    Serial.println("Todos os fios foram cortados!");
  }
}
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (previousState == true && currentState == false) {     
      ...
      if (i == correctWireIndex) {
        ...
        errorCount++;
        Serial.print("ERRO! Fio errado cortado. Total de erros: ");
        Serial.println(errorCount);
        Serial.print("Deveria ter cortado: ");
        Serial.println(wireNames[correctWireIndex]);
        updateErrorLEDs(errorCount);
...
// Helper function to update error LEDs based on error count
void updateErrorLEDs(int count) {
  if (count >= 1) {
    digitalWrite(LED_ERROR1_PIN, HIGH);
    if (DEBUG_MODE) Serial.println("ERROR1 LED turned ON");
  }
  if (count >= 2) {
    digitalWrite(LED_ERROR2_PIN, HIGH);
    if (DEBUG_MODE) Serial.println("ERROR2 LED turned ON");
  }
  if (count >= 3) {
    digitalWrite(LED_ERROR3_PIN, HIGH);
    if (DEBUG_MODE) Serial.println("ERROR3 LED turned ON - Maximum errors reached!");
  }
  
  if (DEBUG_MODE) logLEDStatus();
}
