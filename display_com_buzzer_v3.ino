/* ********************************************************************** 
 * 4-Digit Countdown Timer (30:00 to 00:00)
...
const int dataPin  = 2;   // DS pin (serial data)
const int latchPin = 3;   // STCP pin (storage register clock)
const int clockPin = 4;    // SHCP pin (shift register clock)
const int buzzerPin = 10;  // Digital pin for the buzzer

// LED pins
const int ledPins[] = {6,7,8};  // Red LEDs connected to pins 3, 4, and 5
...
// New function to handle LED activation
...
void CountSecondTimer() {
    ...
    if (alarmActive) {
        // Check if it's time for a new LED to activate (use beepInterval for timing)
        if (currentMillis - lastBeepTime >= beepInterval) {
            lastBeepTime = currentMillis;
            ActivateNextLed();
        }
